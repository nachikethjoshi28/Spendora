...
interface TransactionDao {
    ...
    @Query("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type IN ('SALARY', 'GIFT') AND status != 'DELETED'")
    ...
    @Query("""
        SELECT COALESCE(SUM(CASE 
                   ...
                   END), 0) 
        FROM transactions 
        WHERE type IN ('EXPENSE', 'OTHER') AND status != 'DELETED'
    """)
    ...
    @Query("""
        SELECT SUM(CASE 
                   ...
                   END)
        FROM transactions 
        WHERE UPPER(TRIM(friendName)) = UPPER(TRIM(:friendName)) AND status != 'DELETED'
    """)
    fun getFriendBalance(friendName: String): Flow<Double?>
